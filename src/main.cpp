...
void gpioa_en()
{
    ...
    GPIO_Device<GPIO_Port::A, 5>::getInstance().setOutputPushPull(
        PinFrequency::F_10MHz);
}
...
void usart2_gpio_tx_en()
{
    ...
    GPIO_Device<GPIO_Port::A, 2>::getInstance().setAlternatePushPull(
        PinFrequency::F_50MHz);
}

void usart1_gpio_tx_en()
{
    using namespace Device;
    GPIO_Device<GPIO_Port::A, 9>::getInstance().setAlternatePushPull(
        PinFrequency::F_50MHz);
}
...
int main(void)
{
    ...
    //		  	  	  	  	  	  	 std::bind(&Program::BluetoothEventObserver::handle
    //, observer , _1) );
    //
    //
